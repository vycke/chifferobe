...
describe('Jobbel tests', () => {
  it('simple execution of a queue of jobs', async () => {
    ...
    expect(manager.status).toEqual({ pending: 2, resolved: 0, rejected: 0 });
    await wait(10);
    expect(callback.mock.calls.length).toBe(2);
    expect(manager.status).toEqual({ pending: 2, resolved: 2, rejected: 0 });
    await wait(10);
    expect(callback.mock.calls.length).toBe(4);
    expect(manager.status).toEqual({ pending: 2, resolved: 4, rejected: 0 });
    await wait(10);
    expect(callback.mock.calls.length).toBe(5);
    expect(manager.status).toEqual({ pending: 2, resolved: 5, rejected: 0 });
    await wait(10);
    expect(callback.mock.calls.length).toBe(7);
    expect(manager.status).toEqual({ pending: 2, resolved: 6, rejected: 1 });
    await wait(30);
    expect(callback.mock.calls.length).toBe(9);
    expect(manager.status).toEqual({ pending: 1, resolved: 8, rejected: 1 });
    await wait(50);
    expect(callback.mock.calls.length).toBe(10);
    expect(manager.status).toEqual({ pending: 0, resolved: 9, rejected: 1 });

    manager.reset();
    expect(manager.status).toEqual({ pending: 0, resolved: 0, rejected: 0 });
  });
